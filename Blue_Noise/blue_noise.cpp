...
struct SImageDataComplex
{
	...
	size_t m_width;
	size_t m_height;
	stdvectorstdcomplexfloat m_pixels;
};

======================================================================================
stdcomplexfloat DFTPixel(const SImageData &srcImage, size_t K, size_t L)
{
	stdcomplexfloat ret(0.0f, 0.0f);
...
void DFTImage(const SImageData &srcImage, SImageDataComplex &destImage)
{
	...
	size_t numThreads = stdthreadhardware_concurrency();
	if (numThreads  0)
...
void GetMagnitudeData(const SImageDataComplex& srcImage, SImageData& destImage)
{
	...
	for (size_t x = 0; x srcImage.m_width; ++x)
	{
		for (size_t y = 0; y srcImage.m_height; ++y)
		{
			...
			float mag = stdabs(src);
			if (mag  maxmag)
				maxmag = mag;

			magArray[ysrcImage.m_width + x] = mag;
		}
	}
	if (maxmag == 0.0f)
		maxmag = 1.0f;
...
void ImageInit(SImageData& image, size_t width, size_t height)
{
	...
	stdfill(image.m_pixels.begin(), image.m_pixels.end(), 0);
}
...
void SampleTest(const SImageData& image, const SImageData& samples, const char fileName)
{
	...
	for (size_t y = 0; y image.m_height; ++y)
	{
		...
		for (size_t x = 0; x image.m_width; ++x)
		{
			...
			const SColor samplePixel = (SColor)&samples.m_pixels[sampleYsamples.m_pitch + sampleX  3];
			const SColor imagePixel = (SColor)&image.m_pixels[yimage.m_pitch + x  3];

			SColor outPixel = (SColor)&outImage.m_pixels[youtImage.m_pitch + x  3];
...
int main(int argc, char argv)
{
	...
	white noise
	{
		...
		for (size_t i = 1; i = samples3; ++i)
		{
			...
			SColor pixel = (SColor)&samples.m_pixels[ysamples.m_pitch + x  3];
			pixel - R = pixel - G = pixel - B = 255;

			if (i == samples1  i == samples2  i == samples3)
			{
				printf(White Noise %zu samplesn, i);
		...
		regular samples
	{
		...
		auto GridTest = [&](size_t sampleCount) {
			...
			size_t side = size_t(stdsqrt(float(sampleCount)));
			...
			for (size_t y = 0; y side; ++y)
			{
				...
				for (size_t x = 0; x side; ++x)
				{
					...
					SColor pixel = (SColor)&samples.m_pixels[pixelYsamples.m_pitch + pixelX  3];
					pixel - R = pixel - G = pixel - B = 255;
				}
			}

			printf(Regular %zu samplesn, sampleCount);
		...
		blue noise
	{
		...
		stdvectorstdarraysize_t, 2 samplesPos;
		...
		for (size_t i = 1; i = samples3; ++i)
		{
			...
			SColor pixel = (SColor)&samples.m_pixels[bestCandidateYsamples.m_pitch + bestCandidateX  3];
			pixel - R = pixel - G = pixel - B = 255;

			if (i == samples1  i == samples2  i == samples3)
			{
				printf(nBlue Noise %zu samplesn, i);
