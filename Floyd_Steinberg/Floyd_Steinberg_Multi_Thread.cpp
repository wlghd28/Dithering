...
int main(void)
{
	...
	*/
	...
	{
		OneCpuCal(i, 10000);
		sprintf(str, "Dither/Dither(%d,%d).bmp", i, 10000);
		FwriteCPU(str);
		memset(pixE, 0, sizeof(int) * bpl * bph);
		memset(pix, 0, sizeof(unsigned char) * bpl * bph);
	...
	*/
	...
	for (int y = 1; y < bph - 1; y++)
	{
		for (int x = 1; x < bpl - 1; x++)
		{
			...
			pixE_2 = pixE[y * bpl + x];
			// 점이 찍히는 순간
			// pixE_1에 해당하는 부분 값이 작아 질수록 경계선은 뚜렷해짐.
			// pixE_1에 해당하는 부분 값이 커질수록 경계선이 번지는 효과가 나옴.
			pix[y * bpl + x] = (pixE_2 - (pixE_1 * v1 / vsum)) / 128 * 255;
		}
	}
	*/
